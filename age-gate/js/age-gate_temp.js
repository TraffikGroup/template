...
(function (win) {
	...
	win.agegate = function (args) {
		...
		var options = {
			...
			invalid_country_error: 	args.invalid_country_error || "Sorry, access from this country is not permitted. For more information about Guinness visit <a href='http://guinness.com'>guinness.com</a>",
			...
			default_country:		args.default_country || "CA",
				...
				if (!enabled()) {
					...
					if ((passtoken && passtoken.pass===true) || options.bypass===true) {
						...
						$(options.age_gate_element).fadeOut("fast", function() {
							$(options.curtain_element).fadeOut("fast",function(){$("this").remove();});
							$(this).remove();
						...
						age_gate.load('age-gate/age-gate.html', function() {
							...
							if (options.use_textboxes) {
								$(".dob-inputs").load("age-gate/dob-inputs-text.html", function() {
									//hide the loading animation
									$(".age-gate .loader").hide().remove();
								...
								$(".dob-inputs").load("age-gate/dob-inputs-select.html", function() {
									//hide the loading animation
									$(".age-gate .loader").hide().remove();
							...
							function runGateway() {
								...
								inputs.submit.click(function(e) {
									...
									var values = {
										...
										rememberme: inputs.rememberme.attr("checked")
									}
								
									//if the country is permitted
									if(validateCountry(values.country) && (values.country=="CA") && (values.province!="0")){



										//if the DOB meets LDA
										if(validateDOB(values.day, values.month, values.year, values.country, values.province, options.use_textboxes)) {
											...
											if(values.rememberme !== undefined) {
												setCookie('diageo-age-gate', JSON.stringify(cookie_data), options.domain, options.cookie_expiry_days);
											} else {
												setCookie('diageo-age-gate', JSON.stringify(cookie_data), options.domain);
											}
